...
! function returns version of fortran source code
...
! function returns release of fortran source code
...
! function returns maximum size of variables
...
! inputs
      INTEGER(KIND=4), INTENT(IN)  ::  kext, options(5)
      INTEGER(KIND=4), INTENT(IN)  ::  sysaxes, iyearsat, idoy
      REAL(KIND=8), INTENT(IN)  ::     UT
      REAL(KIND=8), INTENT(IN)  ::     xIN1,xIN2,xIN3
      REAL(KIND=8), INTENT(IN)  ::     maginput(25)
! inout
      REAL(KIND=8), INTENT(INOUT)   ::  posit(3,NREBMAX,NDER_DEF*(options(4)+1))
      REAL(KIND=8), INTENT(INOUT)   ::  BLOCAL(NREBMAX,NDER_DEF*(options(4)+1))
      INTEGER*4, INTENT(INOUT)  ::   ind(NDER_DEF*(options(4)+1))
! outputs
      REAL(KIND=8), INTENT(OUT)  ::   BMIN,XJ
      REAL(KIND=8), INTENT(OUT)  ::   Lm,Lstar
...
!c
      ...
      CALL check_dims_real8_3d(posit, (/3, NREBMAX, NDER/), "posit")
      CALL check_dims_real8_2d(BLOCAL, (/NREBMAX, NDER/), "BLOCAL")
      CALL check_dims_int_1d(ind, (/NDER/), "ind")
...
!c
        write(6,*) "t_resol", t_resol, "r_resol", r_resol
        CALL trace_drift_shell_opt(xGeo &
          ,Lm,Lstar,XJ,BLOCAL,BMIN, &
          posit,ind, t_resol, r_resol)
      END
